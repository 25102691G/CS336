...
def evaluate_vllm(
    ...
    """
    评估语言模型在一系列提示上的表现，计算奖励，并返回每个示例的行。
    """
    ...
    for start in range(0, len(prompts), request_batch_size):
        ...
        for i, out in enumerate(outputs):
            prompt = out.prompt
            text = out.outputs[0].text  # 生成的续写内容
            full_response = text

            gt = batch_gts[i]
            scores = reward_fn(full_response, gt)  # 包含reward/format_reward/answer_reward的字典
...
def main():
    ...
    # MATH jsonl 通常包含 "problem" / "question" 和 "answer" 或类似字段
